...
#for i in range(0, 10, 2):# learning code
    #print(i)
    #print("increased i", i)
...
k1 = (W*L)/(del_x*del_y) - (W1*L1)/(del_x*del_y) # Actual numbers of cells in the control volume.
...
while rmsphi > 0.0001:
    ...
    if rmsphi < 0.0001:
        ...
        #target_phi.write(f"{.5*del_x}   {.5*del_y}   {phi[2,2]}\n")
        #target_phi.write(f"{.5*del_x}    {W-.5*del_y}   {phi[2,jmax-1]}\n")
        #target_phi.write(f"{L-.5*del_x}  {.5*del_y}   {phi[imax-1,2]}\n")
        #target_phi.write(f"{L-.5*del_x} {W-.5*del_y}    {phi[imax-1,jmax-1]}\n")
