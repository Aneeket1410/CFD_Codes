...
W = float(W)
t = input("What is the thickness of the plate? \n >")
t = float(t)
...
for n in range(1,iter):
    ...
    for i,j in corner_points:
        if i==1 and j==1:
            ...
            #worksheet.write((row_counter + i), (column_counter + j), phi_boundary_new)
        elif i==1 and j==jmax:
            phi_boundary_new = float(phi[i,j])
            #worksheet.write((row_counter + i), (column_counter + j), phi_boundary_new)
        elif i==imax and j==1:
    ...
    if n == iter-1:
        ...
        target = open("temperature_data.txt", "w")
